...
<img src="https://github.com/user-attachments/assets/5f458824-c33b-4bc1-8266-46475f5b1be9" />
<img  src="https://github.com/user-attachments/assets/1ff756ae-f0f2-430a-a8df-b93828921648" />
<img  src="https://github.com/user-attachments/assets/5607e1b4-590c-40e5-850b-19ac416cea15" />
<img  alt="image" src="https://github.com/user-attachments/assets/6f295030-a71a-4698-aae8-6a339848cc34" />
...
| **Embedding (arranging)** | A numeric representation of text (or images), that captures meaning, so a computer can compare "similarity" | "king" and "queen" have embeddings that sit mathematically close together |
...
Prompt: *"Complete this sentence: The sun rises in the..."*. 

The model predicts the next token is likely "east" because, during training, it encountered that phrase countless times. It doesn't retrieve the answer from a fact database or reason symbolically—it predicts the most likely next token based on patterns learned from its training data.
...
2. **Augment (enhance):** those retrieved passages get inserted into the prompt as extra context, alongside the original question.
...
```
<img width="1063" height="362" alt="image" src="https://github.com/user-attachments/assets/79c97cd8-c5aa-4b00-8fc0-65fb57b7a939" />
...
## Ollama - A Free Local LLM


**Ollama** is a free tool that lets you **download, run, and interact with Large Language Models (LLMs) locally on your own computer**. Instead of sending your prompts to cloud services like ChatGPT or Gemini, Ollama runs the model directly on your machine.
...
* **Keeps your data private** since prompts stay on your computer
* **Simple API** for developers to integrate with Python, Java, JavaScript, etc.
...
# Download a model (around 4 GB)
ollama pull llama3.1

# (around 1.6 GB) - Im using this
ollama pull gemma2:2b

#Shows list of available LLMs in Local
ollama list

# See only currently running models
ollama ps
...
By default, it starts a local server at: `http://localhost:11434`
...
| Model       | Good For                   |
| ----------- | -------------------------- |
| llama3.1    | General-purpose assistant  |
| gemma3      | Google's lightweight model |
| mistral     | Fast inference             |
| qwen3       | Coding and reasoning       |
...
RAG (Retrieval-Augmented Generation) combines an LLM with external
knowledge...
```
...
Your Python code sends the prompt to the local Ollama server, which runs the model and returns the generated text.
...
| Feature           | Ollama                   | ChatGPT API             |
| ----------------- | ------------------------ | ----------------------- |
| Runs locally      | ✅                        | ❌                       |
| Internet required | Only for model download  | ✅                       |
| API Key           | ❌                        | ✅                       |
| Privacy           | Excellent (local)        | Cloud                   |
| Cost              | Free                     | Paid                    |
| Speed             | Depends on your hardware | Depends on internet/API |

---

## When should you use Ollama?

Use Ollama when you want to:

* Learn LLMs without spending money
* Build AI applications locally
* Keep sensitive data on your machine
* Experiment with prompt engineering and RAG
* Develop and test AI applications without relying on cloud APIs

Once you're comfortable with Ollama, moving to cloud-hosted models like the OpenAI API or Gemini API is straightforward because the overall workflow—sending a prompt and receiving generated text—is very similar.
