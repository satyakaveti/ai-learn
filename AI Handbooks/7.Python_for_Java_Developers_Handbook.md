...
```python
def show_all(*args, **kvargs):
    print("args:", args)        # a tuple of all extra positional values
    print("kvargs:", kvargs)    # a dict of all extra named values

show_all(1, 2, 3, name="Rahul", city="Pune")
# args: (1, 2, 3)
# kwargs: {'name': 'Rahul', 'city': 'Pune'}
```
`*args` is similar to Java's `String... args` varargs. Only Single Values or Primary Data Types
`**kvargs` - to pass list,set,Map  kind of Key-value pairs
