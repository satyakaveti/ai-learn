# AI for Backend Developers — Part 1: The Roadmap
*You're already using AI agents every day. Here's what's actually happening underneath — and what we'll unpack together in this series.*

---

## Introduction — Why This Series Exists

I've spent 14 years as a Java/Spring Boot backend developer. Like a lot of engineers in my position, I've watched AI move from "interesting side project" to "industry standard" faster than almost any shift I've seen in my career — and I decided it was time to actually understand it, not just use it.

I already use ChatGPT, Claude, Cursor, Gemini, and Google Antigravity daily. But using a tool and understanding how it works are two very different things. So I built a learning roadmap for myself, and this series is me documenting that journey — one concept at a time, explained the way I wish someone had explained it to me: with plain language and examples tied to tools I already use.

**Here's what this series will cover, one article per topic:**

- **Part 2 — AI Fundamentals:** What AI, Machine Learning, Deep Learning, and LLMs actually are — and where a lot of us (myself included) get the ML vs DL distinction wrong.
- **Part 3 — Prompt Engineering:** Why prompting is less "magic" and more "writing a clear Jira ticket for an AI."
- **Part 4 — RAG + Vector Databases:** How tools "chat with your documents" — the pipeline behind it, step by step.
- **Part 5 — Agentic AI Concepts:** What actually makes an "agent" different from a chatbot.
- **Part 6 — LangGraph:** How developers build and control multi-step AI agents with precision, instead of leaving it to a black box.
- **Part 7 — MCP (Model Context Protocol):** The new "USB standard" connecting AI models to real-world tools and data.
- **Part 8 — CrewAI:** When one agent isn't enough — coordinating a team of specialized agents.
- **Part 9 — Capstone:** The actual projects I built along the way, including one in Java/Spring AI, as a working portfolio.

This first article lays out the full roadmap and the mental model that ties everything together. Every part after this will go deeper into one layer of this stack.
...
Remember the RAG pipeline from earlier? You could wrap your vector database (containing your company docs) inside an MCP server, exposing a tool like `search_docs(query)`. Now *any* MCP-compatible AI assistant in your company — Claude, an internal chatbot, a Slack bot — can retrieve from that same knowledge base through one single, standardized connection, instead of each tool needing its own custom RAG implementation.
...
That's the full map. In **Part 2**, we'll slow down and go deep on AI Fundamentals — starting with the ML vs DL distinction above, but going further into how neural networks actually learn, and where generative AI fits into all of it.

If you're a backend developer curious about AI but tired of overhyped, hand-wavy explanations, follow along — this series is written by someone learning this the same way you are: one concept, one honest example at a time.

*— Part 1 of the "AI for Backend Developers" series*
